(() => {
  ...
  const fallbackReveal = () => {
    const nodes = document.querySelectorAll('.river-card,.field-intro,.module-bubble,.liosyna-art,.liosyna-copy,.studio-piece,.principles-copy,.principle,.contact>*');
    ...
    const observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        ], { duration: 650, easing: 'cubic-bezier(.2,.75,.2,1)', fill: 'both' });
  ...
  gsap.from('.liosyna-art .atlas-frame', { autoAlpha: 0, scale: .68, rotation: -8, duration: 1.1, ease: 'back.out(1.4)', scrollTrigger: { trigger: '.liosyna', start: 'top 68%', once: true } });
